...
class OrderController extends AdminBaseController
{
    ...
    public function pushToDelhivery(Request $request, $id)
    {

    $order = Order::findOrFail($id);
        
    // Grab the weight typed by the admin in the popup
    $finalWeight = $request->input('final_weight');
    $length = $request->input('shipment_length');
    $width = $request->input('shipment_width');
    $height = $request->input('shipment_height');
    
    if (strpos($order->order_note, 'Delhivery AWB') !== false) {
        return redirect()->back()->with('unsuccess', 'This order has already been pushed to Delhivery!');
    }
    
    $delhivery = new DelhiveryService();
    // Pass the new weight to the service
    $response = $delhivery->createShipment($order, $finalWeight, $length, $width, $height);
        
        // Edge Case 2: Ensure the root request is successful AND the specific package status is 'Success'
        if (isset($response['success']) && $response['success'] == true && isset($response['packages'][0]['status']) && $response['packages'][0]['status'] == 'Success') {
            
            $awb = $response['packages'][0]['waybill'];
            $order->order_note = $order->order_note . " | Delhivery AWB: " . $awb;
            $order->update();

            return redirect()->back()->with('success', 'Order successfully pushed to Delhivery! AWB: ' . $awb);
        }

        // If it fails, try to grab the exact reason why it failed (e.g. "Pincode unserviceable" or "Duplicate order")
        $errorMsg = 'Unknown Delhivery API Error';
        if (isset($response['packages'][0]['remarks'][0])) {
            $errorMsg = $response['packages'][0]['remarks'][0];
        } elseif (isset($response['error'])) {
            $errorMsg = $response['error'];
        }

        return redirect()->back()->with('unsuccess', 'Failed to push to Delhivery: ' . $errorMsg);
    }
    
    // For getting Label
    public function downloadDelhiveryLabel($id)
    {
        $order = Order::findOrFail($id);

        // Check if the order actually has an AWB number saved
        if (strpos($order->order_note, 'Delhivery AWB:') === false) {
            return redirect()->back()->with('unsuccess', 'No AWB found. Please push to Delhivery first.');
        }

        // Extract the AWB number from the order note
        $parts = explode('Delhivery AWB: ', $order->order_note);
        $awb = trim(end($parts));

        // Call our service to get the PDF link
        $delhivery = new DelhiveryService();
        $response = $delhivery->downloadLabel($awb);

        if (isset($response['packages'][0]['pdf_download_link'])) {
            // Redirect the admin directly to the PDF file to download it!
            return redirect()->away($response['packages'][0]['pdf_download_link']);
        }

        return redirect()->back()->with('unsuccess', 'Could not generate label. Delhivery might still be processing this shipment.');
    }
